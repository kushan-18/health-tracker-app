...
export default function ForgotPasswordPage() {
  ...
  const [isResetMode, setIsResetMode] = useState(() =>
    typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("reset") === "true" : false
  );
  ...
  const onSubmit = async (data: EmailFormData) => {
    ...
    const { error: resetError } = await supabase.auth.resetPasswordForEmail(data.email, {
      redirectTo: `${window.location.origin}/auth/forgot-password?reset=true`,
    });
            ...
            {isResetMode ? (
              resetDone ? (
                <motion.div key="done" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-4">
                  ...
                  <Link href="/auth/login" className="w-full glass text-white py-3 rounded-xl font-medium hover:bg-white/10 transition-all flex items-center justify-center gap-2">
                ...
                <motion.div key="reset" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  ...
                  <form onSubmit={onResetPassword} className="space-y-4">
                    <div>
                      <label htmlFor="new-password" className="block text-sm font-medium text-white/60 mb-1.5">New Password</label>
                      <input id="new-password" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="••••••••" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-emerald-500/50 transition-colors" />
                    </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input id="email" {...register("email")} type="email" placeholder="you@example.com" autoFocus className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-emerald-500/50 transition-colors" />
                    </div>
              ...
              <motion.div key="success" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="text-center py-4">
                ...
                <div className="space-y-3">
                  <button
                    ...
                    disabled={isLoading}
                    className="w-full glass text-white py-3 rounded-xl font-medium hover:bg-white/10 transition-all flex items-center justify-center gap-2"
                  >
                    {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Mail className="w-4 h-4" />} Resend Email
                  </button>
